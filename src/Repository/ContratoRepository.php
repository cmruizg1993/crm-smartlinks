    ...
    */
    ...
    public function generarActivacion($anio, $mes){
        ...
        $sql = "UPDATE contrato c
                    LEFT JOIN factura f ON c.id = f.contrato_id AND f.anio_pago = :anio AND f.mes_pago >= :mes AND (f.estado_sri != 'ANULADA' OR f.estado_sri IS NULL) AND (f.factura_plan > 0 || f.factura_plan IS NULL)
